...
CLASSIFY_TIMEOUT = 60  # segundos (LLM puede tardar)
...
def classify_with_ai(content: str, db: Session) -> Optional[list[dict]]:
    """
    ...
    payload = {"text": content, "existing_groups": existing}
...
def find_entry_to_delete(ai: dict, db: Session) -> Optional[InboxEntry]:
    """
    Dado un resultado de IA con action='delete', busca la InboxEntry que
    mejor coincide con project/subproject/idea para marcarla como discarded.
    """
    group    = (ai.get("group") or "").lower()
    subgroup = (ai.get("subgroup") or "").lower()
    idea       = (ai.get("idea") or "").lower()
    ...
    for entry in candidates:
        ...
        if subgroup and entry_sub != subgroup:
            continue
        if idea and idea not in entry_idea and entry_idea not in idea:
            continue
        return entry
